...
class MediaItems {
  ...
  batchGet(mediaItemIds) {
    return this.transport.get(
      `${constants.BASE_PATH}/:batchGet`,
      mediaItemIds.map((el) => ['mediaItemIds', el])
    );
  }

  async upload(albumId, fileName, fileBuffer, description, requestTimeout = 10000) {
    const url = `${constants.BASE_PATH}/:batchCreate`;
    const token = await this.transport.upload(fileName, fileBuffer, requestTimeout);
      ...
      const newMediaItems = await Promise.all(
        batch.map(async (file) => {
          const token = await this.transport.upload(
            file.name,
            fs.readFileSync(path.join(directoryPath, file.name)),
            requestTimeout
          );
